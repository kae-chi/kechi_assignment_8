...
def generate_ellipsoid_clusters(distance, n_samples=100, cluster_std=0.5):
    ...
    X2 = np.random.multivariate_normal(mean=[0, 0], cov=covariance_matrix, size=n_samples)
...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        loss = -np.mean(y * np.log(prob[:, 1]) + (1 - y) * np.log(prob[:, 0]))
        loss_list.append(loss)

        # Margin width calculation using norm of coefficients
        beta_norm = np.linalg.norm(model.coef_[0])
        margin_width = 2 / beta_norm
        margin_widths.append(margin_width)

        # Display equation and margin width
